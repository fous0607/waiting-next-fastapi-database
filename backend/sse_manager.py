...
class SSEConnectionManager:
    ...
    # --- System Log Channel ---
    def __init__(self):
        # store_id별, role별로 연결된 클라이언트들을 관리
        # Structure: {store_id: {role: Set[asyncio.Queue]}}
        self.active_connections: Dict[str, Dict[str, Set[asyncio.Queue]]] = {}
    ...
    async def connect(self, store_id: str, role: str = 'unknown') -> asyncio.Queue:
        """새로운 SSE 연결 추가 (매장용)"""
        if store_id not in self.active_connections:
            self.active_connections[store_id] = {}
        
        if role not in self.active_connections[store_id]:
            self.active_connections[store_id][role] = set()
            
        queue = asyncio.Queue()
        self.active_connections[store_id][role].add(queue)
        print(f"[SSEManager] Connected: store={store_id}, role={role}. Total roles: {len(self.active_connections[store_id])}")
        return queue

    def disconnect(self, store_id: str, queue: asyncio.Queue, role: str = 'unknown'):
        """SSE 연결 제거 (매장용)"""
        if store_id in self.active_connections:
            if role in self.active_connections[store_id]:
                self.active_connections[store_id][role].discard(queue)
                if not self.active_connections[store_id][role]:
                    del self.active_connections[store_id][role]
    ...
    async def broadcast(self, store_id: str, event_type: str, data: dict = None, franchise_id: str = None, target_role: str = None):
        ...
        - target_role: 특정 역할(admin, board 등)에게만 전송. None이면 모든 역할에게 전송
        """
        message = {
            "event": event_type,
            "data": data or {},
        ...
        # 1. 매장 리스너에게 전송
        if store_id in self.active_connections:
            # 타겟 역할이 정해져 있으면 해당 역할만, 아니면 모든 역할
            roles_to_notify = [target_role] if target_role else list(self.active_connections[store_id].keys())
            
            for role in roles_to_notify:
                if role not in self.active_connections[store_id]:
                    continue
                    
                queues = self.active_connections[store_id][role]
                disconnected_queues = []
                ...
                for queue in disconnected_queues:
                    self.disconnect(store_id, queue, role)
